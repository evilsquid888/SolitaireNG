...
public abstract class Rules {
	...
	public void RefreshOptions() {
		mAutoMoveLevel = Integer.parseInt(mView.GetSettings().getString("AutoMoveLevel", "2"));
		mWasFling = false;
	}
...
class NormalSolitaire extends Rules {
	...
	public void Init(Bundle map) {
		...
		if (mView.GetSettings().getBoolean("SolitaireStyleVegas", true)) {
...
class Spider extends Rules {
	...
	public void Init(Bundle map) {
		...
		if (map != null) {
			...
			if (map.getInt("cardAnchorCount") == 12 &&
				...
				suits = map.getInt("suits");
				if (suits == 0)
					suits = Integer.parseInt(mView.GetSettings().getString("SpiderSuits", "4"));
	...
	public void Resize(int width, int height) {
		...
		mCardAnchor[10].SetPosition(-100, 1);
		mCardAnchor[11].SetPosition(-100, 1);
	}
